...
Using the modern Monte Carlo simulator, we swept the bean budget for each synthetic preference regime (uniform, single peak, clustered, polarized, and ``top two'') while holding the cohort size at 80 students. First-choice hit rates quickly plateau once students receive roughly 20 beans, although polarized and single-peak cohorts lag because many participants fight for the same rotation order (Figure \ref{fig:fig1a}). The normalized error follows a similar trajectory—additional beans beyond this threshold yield diminishing returns across every distribution (Figure \ref{fig:fig1b}).
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{First-choice hit rate as a function of beans per student.}
        \label{fig:fig1a}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Normalized average error as a function of beans per student.}
        \label{fig:fig1b}
    \end{subfigure}
    \caption{Impact of bean budgets on first-choice hit rate and normalized error across simulated preference distributions (80 students, 25 runs per distribution).}
    \label{fig:fig1}
\end{figure}

\subsubsection{Preference distribution stress tests}
To quantify how different preference regimes influence allocation quality, we stress-tested the algorithm across five synthetic distributions (uniform, single peak, clustered, polarized, and ``top two'') using the simulator introduced in this repository. Each scenario was run for 25 Monte Carlo repetitions with 80 students and 24 beans per student. Figures \ref{fig:beans_stress} and \ref{fig:linear_stress} summarize the aggregate behavior under the bean-penalty and linear-penalty formulations, respectively. Under bean penalties, clustered and top-two cohorts produced the lowest average error and frequently exceeded a 90\% first-choice hit rate, whereas single-peak and polarized cohorts saturated quickly, capping first-choice satisfaction below 60\%. The linear penalty inverts the magnitude of the measured error but preserves the ordering---clustered cohorts again fare best, while single-peak and polarized preferences remain the most challenging due to heavy contention for a single rotation order.

\begin{figure}[h]
    \centering
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_beans_avg_error.png}
        \caption{Average error by distribution.}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_beans_first_choice.png}
        \caption{First-choice probability.}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_beans_run_trend.png}
        \caption{Average error across runs.}
    \end{subfigure}
    \caption{Stress-test metrics for the bean-penalty formulation across five preference distributions.}
    \label{fig:beans_stress}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_linear_avg_error.png}
        \caption{Average error by distribution.}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_linear_first_choice.png}
        \caption{First-choice probability.}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \centering
        \includegraphics[width=\linewidth]{images/stress_linear_run_trend.png}
        \caption{Average error across runs.}
    \end{subfigure}
    \caption{Stress-test metrics for the linear-penalty formulation using the same synthetic cohorts as Figure \ref{fig:beans_stress}.}
    \label{fig:linear_stress}
\end{figure}
...
Sweeping cohort size while keeping the bean budget fixed at 24 produced steadily decreasing normalized error curves (Figure \ref{fig:fig2a}) while the per-student penalty converged toward a narrow 10--12 bean band once roughly 80 learners were included (Figure \ref{fig:fig2b}). We continued to prioritize the bean-penalty formulation in practice because it lets students express how undesirable a rotation feels rather than forcing a coarse rank.
...
\begin{figure}[h]
    ...
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Normalized average error as a function of cohort size.}
        \label{fig:fig2a}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.49\textwidth}
        ...
        \caption{Average penalty per student as a function of cohort size.}
        \label{fig:fig2b}
    \end{subfigure}
    \caption{Cohort-size sweep (24 beans per student) showing that larger classes reduce normalized error and stabilize the per-student penalty.}
    \label{fig:fig2}
\end{figure}
